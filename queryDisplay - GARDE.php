...
function echoResultTable($result, $colTitles, $item, $data, $selCol, $toolCol) {
    if(!$result) {
		...
		$id = false;
		$reponse = "";
		$reponse .= "<table data-ISAitem='$item' style='margin-left:auto; margin-right:auto; display:none;'>";
		
		if ($colTitles) {   									//   affichage thead
			$titres = $result->fetch_fields();
			$reponse .= "<thead>";
			
			if ($toolCol) $reponse .= "<th style='padding-left:0px; padding-right:8px; border:0px; background:#F9F9F9;'><a  title='Choisir les colonnes'  data-ISAobsId='$obsId' onclick=\"qAjaxTableChooseCols('$item');\" href='#chooseColsPanel' class='ui-btn ui-btn-inline ui-icon-plus ui-btn-icon-notext' style='margin:0px; border:0px;'></a></th>";
...
//..........................................................................................						
    	...
    	while ($row = $result->fetch_array(MYSQLI_NUM)) {
    		...
    		foreach($row as $donn) {
				if ($first && $id) {
					$obsId = $donn;
					$reponse .= "<tr title='Afficher le résumé' data-ISAobsId='$donn'><div class='min-height'><td style='background-color:#F7F7F7;'>&nbsp;</td></div>";
					$first = false;
				}
				else if ($first) {
					$reponse .= "<tr>";
					$reponse .= "<div class='min-height'><td>$donn</td></div>";
					$first = false;
				}
    			else $reponse .= "<div class='min-height'><td>$donn</td></div>";
    		}
			if ($toolCol) {
				...
				$reponse .= "<td style='background:#F9F9F9; padding:0px;'><a title='Supprimer la passation'  data-ISAobsId='$obsId' onclick=\"$('#chooseActionsPanel').attr({'data-isa-id':'$obsId'});\" href='#chooseActionsPanel' class='ui-btn ui-btn-inline ui-icon-delete ui-btn-icon-notext' style='margin:0px; margin-left:0px; padding-right:20px; border:0px;'></a></td>";
			}			
    		$reponse .= "</tr>\n";
    	}
    	$reponse .= "<tr style='height:40px;'></tr></tbody></table>";
    }
	echo $reponse;
}
...
//////////////////////////////////////////////////////////////
////////////////			AFFICHAGE MINI DETAIL
/////////////////////////////////////////////////////////////
function miniDetail($base, $data) {
	$reponse = "<tr style='display:none'><td>&nbsp;</td><td colspan='15'><div style='text-align:left;'><code>";
	...
	if ($row[0]) {
		...
		$reponse .= "<p style='margin-left:16px'>" . $row[0] . "</p>";
	}
	$reponse .= "<strong>Résumé:</strong>";
	...
	for ($i = 1; $i <= 3; $i++) {
		...
		$reponse .= "<div>&nbsp;Ph$i:";
		$reponse .= "&nbsp;∑=&nbsp;<strong>$score</strong>&nbsp;(N=$card),&nbsp;&nbsp;";
		$reponse .= "x&#773;=&nbsp;<strong>$meanScore</strong>";
		if ($i == 1 && $multiM) $meanScore1 = $meanScore;
		if ($i == 2 && $multiM) { // x̅  x&#773;
			$meanScore2 = $meanScore;
			$meanScore12 = round((($meanScore2 + $meanScore1) / 2), 1);
			$reponse .= ",&nbsp;&nbsp;x&#773;<small><sub>(Ph1,Ph2)</sub></small>=&nbsp;<strong>$meanScore12</strong>";
		}
		if ($i == 3 && $multiM) {
			$meanScore123 = round((($meanScore1 + $meanScore2 + $meanScore) / 3), 1);
			$reponse .= ",&nbsp;&nbsp;x&#773;<small><sub>(Ph1,Ph2,Ph3)</sub></small>=&nbsp;<strong>$meanScore123</strong>";
		}
		$reponse .= "</div>";
	}
...
function detail($base, $id) {
	...
	$reponse .= "<strong>Observateur:</strong>&nbsp;" . $row['observateur'] . "<br />";
	$reponse .= "<strong>Participant:</strong>&nbsp;" . $row['participant'] . "<br />";
	$reponse .= "<strong>Période:</strong>&nbsp;" . $row['period'] . "<br />";
	$reponse .= "<strong>Age:</strong>&nbsp;" . $row['ageChrono'] . "<br />";
	$reponse .= "<strong>Age(dév):</strong>&nbsp;" . $row['ageDev'] . "<br />";
	$reponse .= "<strong>Genre:</strong>&nbsp;" . $row['sex'] . "<br />";
	$reponse .= "<strong>Date:</strong>&nbsp;" . $row['date'] . "<br />";
	$reponse .= "<strong>Heure:</strong>&nbsp;" . $row['time'] . "<br />";
	$reponse .= "<strong>Etablissement:</strong>&nbsp;" . $row['location'] . "<br />";
	$reponse .= "<strong>#1:</strong>&nbsp;" . $row['rem1'] . "<br />";
	$reponse .= "<strong>#2:</strong>&nbsp;" . $row['rem2'] . "<br />";
	$reponse .= "<strong>#3:</strong>&nbsp;" . $row['rem3'] . "<br />";
	...
	for ($phase = 1; $phase <= 3; $phase++) {
		...
		for ($item = 1; $item <= 12; $item++) {
			...
			$note = $row[$index];
			if ($note < 0) $note = "-";
			$reponse .= "&nbsp;&nbsp;<strong>Item$itemText:</strong>&nbsp;&nbsp;&nbsp;A=<strong>&nbsp;$note</strong>";
			$index = "phase$phase" . "_score$item" . "_B";
			$note = $row[$index];
			if ($note < 0) $note = "-";
			$reponse .= "&nbsp;&nbsp;&nbsp;B=<strong>&nbsp;$note</strong>";
			$index = "$phase" . "_" . "$item";
			$note = $row[$index];
			if ($note < 0) $note = "-";
			$reponse .= "&nbsp;&nbsp;&nbsp;max(A,B)=&nbsp;<strong>$note</strong>";
			$reponse .= "<br />";
			$index = "ph$phase" . "_$item" . "_rem";
			if ($row[$index]) $reponse .= "&nbsp;&nbsp;&nbsp;&nbsp;<strong>Remarque:</strong><p style='margin-left:32px;'>$row[$index]</p>";
		}
		$reponse .= "<br />";
	}
